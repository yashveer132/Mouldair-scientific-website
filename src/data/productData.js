// Method 1: Named export
export const productData = [
  // Watson Marlow

  {
    id: "wm1",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "Qdos chemical metering and dosing pump",
    description: "",
    imageUrl: ["/watson1.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm2",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "Qdos H-FLO: chemical metering and dosing pump",
    description: "",
    imageUrl: ["/watson2.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm3",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "Quantum single-use bioprocessing pumps",
    description: "",
    imageUrl: ["/watson3.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm4",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "100 series cased pump",
    description: "",
    imageUrl: ["/watson4.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm5",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "300 series cased pump",
    description: "",
    imageUrl: ["/watson5.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm6",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "530 process pumps",
    description: "",
    imageUrl: ["/watson6.jpg"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm7",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "630 mid-flow process pumps",
    description: "",
    imageUrl: ["/watson7.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wm8",
    brand: "watson-marlow",
    pumpType: "Peristaltic Pumps",
    name: "730 process pumps",
    description: "",
    imageUrl: ["/watson8.webp"],
    features: [],
    applications: [],
    technicalSpecs: {},
    additionalFeatures: [],
    datasheetUrl: "",
    videoId: "",
  },

  // Leybold

  {
    id: "ley1",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "TRIVAC B",
    description:
      "TRIVAC B series are robust dual-stage oil-sealed rotary vane pumps designed for reliable vacuum generation. They offer excellent performance in various industrial and research applications, with ultimate pressures down to 10⁻⁴ mbar.",
    imageUrl: ["/trivacb.webp", "/pump1.png"],
    features: [
      "Dual-stage rotary vane technology",
      "Gas ballast valve for vapor handling",
      "Anti-suckback protection",
      "Low noise level operation",
      "Easy maintenance access",
      "Long service intervals",
    ],
    applications: [
      "Vacuum furnaces",
      "Metallurgy",
      "Research and development",
      "Analytical instruments",
      "Electron microscopes",
      "Industrial coating",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "4-65 m³/h",
      "Ultimate Pressure": "≤ 2 × 10⁻⁴ mbar",
      "Operating Temperature": "12-40°C",
      "Noise Level": "52-65 dB(A)",
      "Motor Power": "0.37-2.2 kW",
      "Oil Capacity": "0.5-2 L",
    },
    additionalFeatures: [
      "Integrated oil return system",
      "Thermal protection switch",
      "Direct drive coupling",
      "Available with different motor options",
      "Optional exhaust filters",
    ],
    datasheetUrl: "",
    videoId: "ABC123", // Replace with actual video ID
  },
  {
    id: "ley2",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "TRIVAC L",
    description:
      "TRIVAC L series are single-stage oil-sealed rotary vane pumps designed for general vacuum applications. These pumps combine proven design with modern manufacturing technology to provide reliable vacuum generation.",
    imageUrl: ["/trivacl.webp", "/pump1.png"],
    features: [
      "Single-stage rotary vane design",
      "Built-in oil return system",
      "Gas ballast valve",
      "Anti-suckback valve",
      "Direct drive motor",
      "Air cooling system",
    ],
    applications: [
      "General industrial applications",
      "Vacuum packaging",
      "Heat treatment",
      "Vacuum drying",
      "Food processing",
      "Medical technology",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "4-25 m³/h",
      "Ultimate Pressure": "≤ 2 × 10⁻³ mbar",
      "Operating Temperature": "12-40°C",
      "Noise Level": "50-60 dB(A)",
      "Motor Power": "0.37-0.75 kW",
      "Oil Capacity": "0.3-1.0 L",
    },
    additionalFeatures: [
      "Simple maintenance",
      "Compact design",
      "Robust construction",
      "Long service intervals",
      "Optional oil mist eliminator",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley3",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "SOGEVAC B",
    description:
      "SOGEVAC B series are robust single-stage oil-sealed rotary vane pumps for industrial applications. They offer excellent performance in rough vacuum applications with high reliability and low maintenance requirements.",
    imageUrl: ["/sogevacb.webp", "/pump2.png"],
    features: [
      "Single-stage oil-sealed technology",
      "High water vapor tolerance",
      "Built-in oil recovery system",
      "Large oil volume",
      "Efficient air cooling",
      "Integrated gas ballast",
    ],
    applications: [
      "Industrial processes",
      "Packaging industry",
      "Plastics degassing",
      "Pick and place",
      "Central vacuum systems",
      "Freeze drying",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "10-840 m³/h",
      "Ultimate Pressure": "≤ 5 × 10⁻³ mbar",
      "Water Vapor Tolerance": "30 mbar",
      "Operating Temperature": "12-45°C",
      "Noise Level": "58-85 dB(A)",
      "Oil Capacity": "0.8-27 L",
    },
    additionalFeatures: [
      "Extended oil lifetime",
      "Low power consumption",
      "Robust construction",
      "Easy maintenance access",
      "Optional accessories available",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley4",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "SOGEVAC BI / DI",
    description:
      "SOGEVAC BI/DI series combines proven pump technology with smart integration features for Industry 4.0 compatibility. These pumps offer advanced monitoring and control capabilities for modern industrial applications.",
    imageUrl: ["/sogevacbidi.webp"],
    features: [
      "Integrated smart controls",
      "Industry 4.0 ready",
      "Remote monitoring capability",
      "Variable speed control",
      "Energy efficiency optimization",
      "Predictive maintenance",
    ],
    applications: [
      "Advanced manufacturing",
      "Smart factories",
      "Automated production lines",
      "Process integration",
      "Data-driven operations",
      "Remote monitoring systems",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "10-630 m³/h",
      "Ultimate Pressure": "≤ 5 × 10⁻³ mbar",
      "Interface Options": "Profibus, EtherCAT, Modbus",
      "Control Features": "Speed control, pressure control",
      "Power Range": "0.5-15 kW",
      Communication: "Digital I/O, Fieldbus",
    },
    additionalFeatures: [
      "Real-time monitoring",
      "Data logging capabilities",
      "Smart diagnostics",
      "Energy consumption monitoring",
      "Maintenance scheduling",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley5",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "SOGEVAC FP",
    description:
      "SOGEVAC FP series are food processing-specific vacuum pumps designed to meet strict hygiene requirements. These pumps feature food-grade oils and special sealing systems for safe operation in food packaging and processing environments.",
    imageUrl: ["/sogevacfp.webp"],
    features: [
      "Food-grade oil compatible",
      "Stainless steel fittings",
      "Clean-in-place capability",
      "Hygienic design",
      "HACCP compliant",
      "Easy to clean and maintain",
    ],
    applications: [
      "Food packaging",
      "Modified atmosphere packaging",
      "Vacuum cooling",
      "Meat processing",
      "Dairy industry",
      "Pharmaceutical packaging",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "25-300 m³/h",
      "Ultimate Pressure": "≤ 5 × 10⁻³ mbar",
      "Water Vapor Tolerance": "30 mbar",
      "Temperature Range": "12-45°C",
      "Protection Class": "IP55",
      "Noise Level": "65-75 dB(A)",
    },
    additionalFeatures: [
      "FDA compliant materials",
      "Optional oxygen service",
      "Corrosion resistant",
      "Explosion proof versions available",
      "Integrated gas ballast",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley6",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "NEO (S)D",
    description:
      "NEO (S)D series represents the next evolution in oil-sealed vacuum pumps, offering superior performance with smart diagnostics and Industry 4.0 connectivity. These pumps are designed for demanding industrial applications requiring reliable vacuum generation.",
    imageUrl: ["/neosd.webp"],
    features: [
      "Smart pump technology",
      "Integrated condition monitoring",
      "Advanced oil management",
      "Energy efficient operation",
      "Remote diagnostics capability",
      "Optimized cooling system",
    ],
    applications: [
      "Industrial automation",
      "Semiconductor manufacturing",
      "Solar cell production",
      "Heat treatment",
      "Metallurgy",
      "Chemical processing",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "65-630 m³/h",
      "Ultimate Pressure": "≤ 0.1 mbar",
      "Power Consumption": "1.5-15 kW",
      "Operating Temperature": "10-45°C",
      Communication: "Industrial Ethernet",
      "Control Interface": "Touch display",
    },
    additionalFeatures: [
      "Predictive maintenance",
      "Cloud connectivity",
      "Performance optimization",
      "Data logging and analysis",
      "Energy consumption monitoring",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley7",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "VACUBE",
    description:
      "VACUBE is a centralized vacuum system combining oil-sealed vacuum pumps with smart control technology. It provides efficient and reliable vacuum for multiple points of use while optimizing energy consumption.",
    imageUrl: ["/vacube.webp"],
    features: [
      "Multi-point vacuum supply",
      "Intelligent control system",
      "Variable speed drive",
      "Energy optimization",
      "Automated operation",
      "Modular design",
    ],
    applications: [
      "Central vacuum systems",
      "Packaging lines",
      "Woodworking industry",
      "Plastic forming",
      "Printing and paper",
      "General manufacturing",
    ],
    technicalSpecs: {
      "System Capacity": "100-2000 m³/h",
      "Ultimate Pressure": "≤ 0.5 mbar",
      "Control Range": "1-1000 mbar",
      "Power Range": "2.2-45 kW",
      "Network Integration": "Industry 4.0 ready",
      "Response Time": "< 100ms",
    },
    additionalFeatures: [
      "Load sharing capability",
      "Pressure-based control",
      "Remote monitoring",
      "Multi-pump management",
      "Energy recovery options",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley8",
    brand: "leybold",
    pumpType: "Oil Sealed Vacuum Pumps",
    name: "LEYBA",
    description:
      "LEYBA vacuum pumps are designed for basic industrial applications requiring reliable and cost-effective vacuum generation. These pumps offer simplified operation and maintenance while maintaining essential performance features.",
    imageUrl: ["/leyba.webp"],
    features: [
      "Simplified design",
      "Robust construction",
      "Easy maintenance",
      "Oil recirculation system",
      "Built-in exhaust filter",
      "Compact footprint",
    ],
    applications: [
      "General industrial use",
      "Educational institutions",
      "Research laboratories",
      "Small-scale manufacturing",
      "Maintenance workshops",
      "Testing facilities",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "10-40 m³/h",
      "Ultimate Pressure": "≤ 2 × 10⁻³ mbar",
      "Motor Power": "0.37-1.5 kW",
      "Operating Temperature": "12-40°C",
      "Oil Capacity": "0.5-1.5 L",
      "Noise Level": "58-62 dB(A)",
    },
    additionalFeatures: [
      "Low maintenance costs",
      "Standard motor options",
      "Basic monitoring functions",
      "Universal voltage options",
      "Standard fittings",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley9",
    brand: "leybold",
    pumpType: "Small Dry Pumps",
    name: "DIVAC",
    description:
      "DIVAC pumps are compact, oil-free diaphragm vacuum pumps offering reliable performance in clean applications. These pumps are designed for contamination-free vacuum generation in laboratory and light industrial processes.",
    imageUrl: ["/divac.webp", "/pump3.png"],
    features: [
      "100% oil-free operation",
      "Maintenance-free design",
      "Chemical resistant materials",
      "Compact footprint",
      "Low vibration",
      "Integrated gas ballast valve",
    ],
    applications: [
      "Laboratory equipment",
      "Mass spectrometry",
      "Gas analysis",
      "Medical technology",
      "Clean handling processes",
      "Filtration systems",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "0.6-4.8 m³/h",
      "Ultimate Pressure": "≤ 2 mbar",
      "Power Consumption": "0.1-0.4 kW",
      "Operating Temperature": "10-40°C",
      "Protection Class": "IP40",
      Weight: "6.5-14.5 kg",
    },
    additionalFeatures: [
      "PTFE diaphragm technology",
      "Long diaphragm lifetime",
      "Low noise operation",
      "Multiple voltage options",
      "Optional electromagnetic shielding",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley10",
    brand: "leybold",
    pumpType: "Small Dry Pumps",
    name: "ECODRY plus",
    description:
      "ECODRY plus is a new generation of compact dry compressing vacuum pumps that combines superior performance with a robust, user-friendly design. These pumps offer clean, dry vacuum for research and analytical applications.",
    imageUrl: ["/ecodry.webp"],
    features: [
      "100% oil-free operation",
      "Low noise and vibration",
      "Energy efficient design",
      "Air cooling system",
      "Compact footprint",
      "User-friendly interface",
    ],
    applications: [
      "Mass spectrometry",
      "Electron microscopy",
      "Surface analysis",
      "Research laboratories",
      "Load lock pumping",
      "Analytical instruments",
    ],
    technicalSpecs: {
      "Pumping Speed": "4-40 m³/h",
      "Ultimate Pressure": "< 6 × 10⁻² mbar",
      "Power Consumption": "0.4-1.1 kW",
      "Noise Level": "< 52 dB(A)",
      "Operating Temperature": "10-40°C",
      Weight: "25-45 kg",
    },
    additionalFeatures: [
      "Integrated control electronics",
      "Status LED display",
      "Configurable I/O interface",
      "Temperature management",
      "Low maintenance requirements",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley11",
    brand: "leybold",
    pumpType: "Small Dry Pumps",
    name: "SCROLLVAC plus",
    description:
      "SCROLLVAC plus series are compact, oil-free scroll vacuum pumps offering reliable performance for clean applications. These pumps feature innovative scroll technology with enhanced durability and efficiency.",
    imageUrl: ["/scrollvac.webp"],
    features: [
      "Oil-free scroll technology",
      "Low maintenance design",
      "High water vapor tolerance",
      "Quiet operation",
      "Hermetic sealing",
      "Smart drive system",
    ],
    applications: [
      "Research and development",
      "Analytical equipment",
      "Semiconductor processes",
      "Laser systems",
      "Glove boxes",
      "Helium recovery",
    ],
    technicalSpecs: {
      "Pumping Speed": "5-30 m³/h",
      "Ultimate Pressure": "≤ 1 × 10⁻² mbar",
      "Water Vapor Tolerance": "40 mbar",
      "Power Consumption": "0.4-0.9 kW",
      "Noise Level": "< 55 dB(A)",
      "Operating Temperature": "10-40°C",
    },
    additionalFeatures: [
      "Easy tip seal replacement",
      "Contamination protection",
      "Integrated hour counter",
      "Stand-by mode",
      "Optional inlet filter",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley12",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "CLAWVAC B",
    description:
      "CLAWVAC B is a robust dry claw vacuum pump designed for heavy-duty industrial applications. It offers reliable performance with minimal maintenance requirements and excellent energy efficiency.",
    imageUrl: ["/clawvacb.webp"],
    features: [
      "100% oil-free operation",
      "Robust claw technology",
      "Air-cooled design",
      "Low maintenance requirements",
      "Variable speed control option",
      "Compact footprint",
    ],
    applications: [
      "Pneumatic conveying",
      "Packaging machines",
      "Woodworking industry",
      "Plastics processing",
      "Central vacuum systems",
      "Industrial automation",
    ],
    technicalSpecs: {
      "Pumping Speed": "65-300 m³/h",
      "Ultimate Pressure": "150 mbar",
      "Motor Power": "1.5-5.5 kW",
      "Noise Level": "65-75 dB(A)",
      "Operating Temperature": "0-40°C",
      "Protection Class": "IP55",
    },
    additionalFeatures: [
      "Smart control interface",
      "Integrated inlet filter",
      "Robust cast iron construction",
      "Self-draining design",
      "Easy access for maintenance",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley13",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "DRYVAC",
    description:
      "DRYVAC series are high-performance dry compressing screw vacuum pumps engineered for demanding industrial processes. They combine superior vacuum performance with advanced control capabilities.",
    imageUrl: ["/dryvac.webp", "/pump4.png"],
    features: [
      "Dry screw technology",
      "Innovative rotor design",
      "Advanced temperature management",
      "Integrated monitoring system",
      "Low energy consumption",
      "Industry 4.0 ready",
    ],
    applications: [
      "Semiconductor manufacturing",
      "Solar cell production",
      "Display manufacturing",
      "Chemical processing",
      "Industrial coating",
      "Heat treatment",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "200-1600 m³/h",
      "Ultimate Pressure": "< 0.01 mbar",
      "Power Consumption": "3.5-30 kW",
      "Water Vapor Capacity": "25 kg/h",
      "Control Interface": "PLC/Fieldbus",
      "Operating Temperature": "5-45°C",
    },
    additionalFeatures: [
      "Remote monitoring capability",
      "Pressure control system",
      "Data logging and analysis",
      "Modular design concept",
      "Emergency power backup options",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley14",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "DRYVAC DV FP-r",
    description:
      "DRYVAC DV FP-r is a specialized dry pump designed for demanding flat panel display and solar manufacturing processes. It features enhanced corrosion resistance and superior process compatibility.",
    imageUrl: ["/dryvacdv.webp"],
    features: [
      "Corrosion-resistant design",
      "Process-optimized operation",
      "Advanced coating technology",
      "Integrated purge system",
      "Smart monitoring",
      "High throughput capability",
    ],
    applications: [
      "Flat panel display production",
      "Solar cell manufacturing",
      "OLED processing",
      "Thin film deposition",
      "Clean technology processes",
      "Advanced materials processing",
    ],
    technicalSpecs: {
      "Pumping Speed": "400-2000 m³/h",
      "Ultimate Pressure": "< 0.005 mbar",
      "Power Rating": "7.5-37 kW",
      "Purge Gas Flow": "0-60 slm",
      Interface: "EtherCAT/Profibus",
      "Protection Class": "IP54",
    },
    additionalFeatures: [
      "Process recipe control",
      "Reactive gas handling",
      "Advanced diagnostics",
      "Predictive maintenance",
      "High uptime reliability",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley15",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "DURADRY",
    description:
      "DURADRY is a rugged dry pump designed for harsh industrial environments and demanding processes. It features superior durability and resistance to aggressive media while maintaining consistent performance.",
    imageUrl: ["/duradry.webp"],
    features: [
      "Harsh process compatible",
      "Advanced corrosion protection",
      "High dust tolerance",
      "Self-cleaning mechanism",
      "Robust shaft sealing",
      "Optimized thermal management",
    ],
    applications: [
      "Metallurgy processes",
      "Chemical processing",
      "Coating applications",
      "Plasma treatment",
      "Industrial drying",
      "Heat treatment",
    ],
    technicalSpecs: {
      "Pumping Speed": "200-1000 m³/h",
      "Ultimate Pressure": "< 0.01 mbar",
      "Power Rating": "5.5-22 kW",
      "Temperature Range": "5-45°C",
      "Protection Class": "IP55",
      "Noise Level": "< 75 dB(A)",
    },
    additionalFeatures: [
      "Particulate handling system",
      "Temperature monitoring",
      "Integrated purge control",
      "Horizontal or vertical mounting",
      "Low maintenance intervals",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley16",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "LEYVAC",
    description:
      "LEYVAC is a versatile dry compressing vacuum pump designed for industrial applications requiring reliable and stable vacuum performance. It offers excellent value with robust construction and proven technology.",
    imageUrl: ["/leyvac.webp"],
    features: [
      "Oil-free compression",
      "Low operating costs",
      "Minimal maintenance",
      "Compact design",
      "Air cooling system",
      "Universal voltage range",
    ],
    applications: [
      "Industrial coating",
      "Furnace applications",
      "Vacuum drying",
      "Crystal pulling",
      "Electron beam welding",
      "LED manufacturing",
    ],
    technicalSpecs: {
      "Pumping Speed": "80-700 m³/h",
      "Ultimate Pressure": "< 0.1 mbar",
      "Power Consumption": "2.2-15.0 kW",
      "Operating Temperature": "0-40°C",
      "Start-up Time": "< 3 minutes",
      "Maintenance Interval": "20,000 hours",
    },
    additionalFeatures: [
      "Integrated exhaust silencer",
      "Variable frequency drive option",
      "Status monitoring",
      "Self-protecting operation",
      "Easy installation",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley17",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "NOVADRY",
    description:
      "NOVADRY is a new generation of dry screw vacuum pumps specifically designed for food and packaging applications. It offers oil-free operation with superior performance and reliability in demanding environments.",
    imageUrl: ["/novadry.webp"],
    features: [
      "100% oil-free operation",
      "Food-grade design",
      "Low maintenance needs",
      "Air-cooling system",
      "Built-in silencer",
      "HACCP compliant",
    ],
    applications: [
      "Food packaging",
      "Meat processing",
      "Fish processing",
      "Dairy industry",
      "Ready meals production",
      "Pharmaceutical packaging",
    ],
    technicalSpecs: {
      "Pumping Speed": "200-900 m³/h",
      "Ultimate Pressure": "< 0.1 mbar",
      "Power Consumption": "3.5-18.5 kW",
      "Noise Level": "< 70 dB(A)",
      "Operating Temperature": "0-40°C",
      "Protection Class": "IP55",
    },
    additionalFeatures: [
      "Hygienic design",
      "Easy to clean",
      "Stainless steel fittings",
      "Self-draining design",
      "Optional oxygen operation",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley18",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "SCREWLINE SP",
    description:
      "SCREWLINE SP is a robust dry screw vacuum pump engineered for demanding industrial processes. It features a unique screw design optimized for harsh applications and continuous operation.",
    imageUrl: ["/screwlinesp.webp"],
    features: [
      "Heavy-duty construction",
      "Advanced screw geometry",
      "High vapor tolerance",
      "Integrated monitoring",
      "Modular design",
      "Variable speed control",
    ],
    applications: [
      "Steel degassing",
      "Vacuum metallurgy",
      "Industrial coating",
      "Heat treatment",
      "Vacuum drying",
      "Plasma nitriding",
    ],
    technicalSpecs: {
      "Pumping Speed": "250-630 m³/h",
      "Ultimate Pressure": "< 0.01 mbar",
      "Power Rating": "7.5-15 kW",
      "Water Vapor Capacity": "30 kg/h",
      "Operating Temperature": "5-45°C",
      "Maintenance Interval": "16,000 hours",
    },
    additionalFeatures: [
      "Harsh process compatibility",
      "Remote monitoring options",
      "Customizable configurations",
      "High uptime reliability",
      "Easy serviceability",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley19",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "VARODRY",
    description:
      "VARODRY is an innovative dry compressing vacuum pump designed for industrial applications. It offers reliable performance with low maintenance requirements and excellent energy efficiency.",
    imageUrl: ["/varodry.webp"],
    features: [
      "100% oil-free operation",
      "Energy-efficient design",
      "Low noise emission",
      "Compact footprint",
      "Air-cooled system",
      "Robust construction",
    ],
    applications: [
      "Pick and place",
      "Packaging industry",
      "Plastics processing",
      "Woodworking",
      "Printing systems",
      "General industrial use",
    ],
    technicalSpecs: {
      "Pumping Speed": "65-160 m³/h",
      "Ultimate Pressure": "< 0.01 mbar",
      "Power Consumption": "1.5-3.0 kW",
      "Noise Level": "< 65 dB(A)",
      "Operating Temperature": "0-40°C",
      "Protection Class": "IP55",
    },
    additionalFeatures: [
      "Variable speed drive option",
      "Industry 4.0 ready",
      "Built-in monitoring",
      "Easy maintenance access",
      "Long service intervals",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley20",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "NOVADRY NDi System",
    description:
      "NOVADRY NDi System is an integrated vacuum solution featuring advanced control capabilities and smart monitoring for food packaging and processing applications.",
    imageUrl: ["/novadryndi.webp"],
    features: [
      "Integrated control system",
      "Smart monitoring",
      "Food-grade design",
      "Multiple pump control",
      "Energy management",
      "Remote operation",
    ],
    applications: [
      "Food packaging lines",
      "Modified atmosphere packaging",
      "Tray sealing",
      "Thermoforming",
      "Multi-chamber machines",
      "Central vacuum systems",
    ],
    technicalSpecs: {
      "System Capacity": "200-1800 m³/h",
      "Control Range": "1-1000 mbar",
      "Power Management": "Load sharing",
      Communication: "Industrial Ethernet",
      Interface: "Touch panel HMI",
      "Protection Class": "IP54",
    },
    additionalFeatures: [
      "Pressure control",
      "Energy optimization",
      "Maintenance scheduling",
      "Data logging",
      "System redundancy",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley21",
    brand: "leybold",
    pumpType: "Industrial Dry Vacuum Pumps",
    name: "VARODRY VDi System",
    description:
      "VARODRY VDi System is a complete vacuum solution with integrated controls designed for industrial applications requiring reliable and efficient vacuum generation.",
    imageUrl: ["/varodryvdi.webp"],
    features: [
      "Integrated pump control",
      "Energy efficiency system",
      "Smart monitoring",
      "Multi-pump management",
      "Pressure control",
      "Remote connectivity",
    ],
    applications: [
      "Industrial automation",
      "Packaging systems",
      "Process integration",
      "Centralized vacuum",
      "Manufacturing lines",
      "Assembly automation",
    ],
    technicalSpecs: {
      "System Capacity": "65-480 m³/h",
      "Ultimate Pressure": "< 0.01 mbar",
      "Control Interface": "PLC/Fieldbus",
      "Power Management": "Smart control",
      "Operating Modes": "Multiple",
      "Network Protocol": "Industrial Ethernet",
    },
    additionalFeatures: [
      "Load balancing",
      "Predictive maintenance",
      "Energy monitoring",
      "System diagnostics",
      "Flexible configuration",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley22",
    brand: "leybold",
    pumpType: "Roots Blowers",
    name: "RUVAC WH / WHU",
    description:
      "RUVAC WH/WHU series are high-performance Roots vacuum pumps designed for industrial applications requiring high pumping speeds. These pumps feature advanced rotor technology and reliable operation.",
    imageUrl: ["/ruvacwh.webp", "/pump5.png"],
    features: [
      "Advanced rotor design",
      "High pumping speed",
      "Frequency converter ready",
      "Water-cooled option",
      "Integrated bypass valve",
      "Robust construction",
    ],
    applications: [
      "Vacuum furnaces",
      "Metallurgy",
      "Coating systems",
      "Chemical processes",
      "Industrial drying",
      "Large chamber evacuation",
    ],
    technicalSpecs: {
      "Pumping Speed": "280-8000 m³/h",
      "Ultimate Pressure": "< 1 × 10⁻³ mbar",
      "Motor Power": "0.75-22 kW",
      "Max Pressure Difference": "80-110 mbar",
      "Operating Temperature": "5-40°C",
      "Protection Class": "IP55",
    },
    additionalFeatures: [
      "Multiple mounting positions",
      "Low maintenance design",
      "Synchronized gear drive",
      "Emergency venting option",
      "Various flange sizes",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley23",
    brand: "leybold",
    pumpType: "Roots Blowers",
    name: "RUVAC WA / WAU",
    description:
      "RUVAC WA/WAU series are compact Roots pumps offering efficient vacuum performance for medium-sized applications. They combine reliability with cost-effective operation.",
    imageUrl: ["/ruvacwa.webp"],
    features: [
      "Compact design",
      "Energy efficient operation",
      "Low noise level",
      "Easy maintenance",
      "Flexible installation",
      "Standard motor drive",
    ],
    applications: [
      "Industrial processes",
      "Research applications",
      "Load lock systems",
      "Vacuum drying",
      "Backing pumps",
      "General vacuum systems",
    ],
    technicalSpecs: {
      "Pumping Speed": "250-4000 m³/h",
      "Ultimate Pressure": "< 1 × 10⁻³ mbar",
      "Motor Rating": "0.55-11 kW",
      "Max Pressure Difference": "50-80 mbar",
      "Rotation Speed": "1500-3000 rpm",
      "Weight Range": "26-520 kg",
    },
    additionalFeatures: [
      "Direct drive system",
      "Air or water cooling",
      "Integrated safety valve",
      "Multiple accessories",
      "Standardized connections",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley24",
    brand: "leybold",
    pumpType: "Roots Blowers",
    name: "RUVAC WS / WSU",
    description:
      "RUVAC WS/WSU series are specialized Roots pumps designed for high-throughput applications. These pumps feature enhanced performance capabilities and robust construction for demanding processes.",
    imageUrl: ["/ruvacws.webp"],
    features: [
      "High throughput design",
      "Enhanced compression ratio",
      "Specialized sealing system",
      "Heavy-duty bearings",
      "Industrial drive system",
      "Process-optimized design",
    ],
    applications: [
      "Large vacuum chambers",
      "Process industry",
      "Semiconductor manufacturing",
      "Solar cell production",
      "Plasma systems",
      "High-energy physics",
    ],
    technicalSpecs: {
      "Pumping Speed": "2000-12000 m³/h",
      "Ultimate Pressure": "< 1 × 10⁻³ mbar",
      "Power Range": "5.5-30 kW",
      "Max Compression Ratio": "10:1",
      "Operating Temperature": "5-40°C",
      "Control Interface": "PLC compatible",
    },
    additionalFeatures: [
      "Advanced monitoring",
      "Customizable configurations",
      "High reliability design",
      "Extended service intervals",
      "System integration ready",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley25",
    brand: "leybold",
    pumpType: "Turbomolecular Vacuum Pumps",
    name: "TURBOVAC i",
    description:
      "TURBOVAC i series are high-performance turbomolecular pumps with integrated drive electronics. These pumps provide excellent vacuum performance and reliability for demanding high and ultra-high vacuum applications.",
    imageUrl: ["/turbovaci.webp", "/pump6.png"],
    features: [
      "Integrated controller and drive electronics",
      "Advanced rotor design",
      "Hybrid bearing system",
      "Temperature management system",
      "Smart monitoring capabilities",
      "Compact design",
    ],
    applications: [
      "Analytical instruments",
      "Mass spectrometry",
      "Electron microscopes",
      "Surface analysis",
      "R&D systems",
      "Semiconductor processing",
    ],
    technicalSpecs: {
      "Pumping Speed Range": "90-1450 l/s (N₂)",
      "Ultimate Pressure": "< 1 × 10⁻¹⁰ mbar",
      "Compression Ratio": "> 10¹¹ (N₂)",
      "Max Backing Pressure": "20 mbar",
      "Rotation Speed": "27,000-90,000 rpm",
      "Power Consumption": "24-300W",
    },
    additionalFeatures: [
      "USB and RS485 interfaces",
      "Remote monitoring and control",
      "Field-proven reliability",
      "Venting options",
      "Various flange sizes available",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley26",
    brand: "leybold",
    pumpType: "Turbomolecular Vacuum Pumps",
    name: "TURBOVAC MAG",
    description:
      "TURBOVAC MAG series are advanced magnetic bearing turbomolecular pumps designed for ultra-high vacuum applications. These pumps offer maintenance-free operation with superior reliability.",
    imageUrl: ["/turbovacmag.webp"],
    features: [
      "Magnetic bearing technology",
      "Zero maintenance bearings",
      "High compression ratios",
      "Advanced rotor design",
      "Integrated electronics",
      "Wide range operation",
    ],
    applications: [
      "Research facilities",
      "Particle accelerators",
      "Surface science",
      "Space simulation",
      "Semiconductor processing",
      "Thin film deposition",
    ],
    technicalSpecs: {
      "Pumping Speed": "250-3000 l/s",
      "Ultimate Pressure": "< 1 × 10⁻¹⁰ mbar",
      "Compression Ratio": "> 10¹² for N₂",
      "Rotation Speed": "30,000-60,000 rpm",
      "Power Consumption": "100-1000W",
      "Bearing Type": "Active magnetic",
    },
    additionalFeatures: [
      "Maintenance-free operation",
      "Advanced diagnostics",
      "Multiple interface options",
      "Backing pump protection",
      "Temperature management",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley27",
    brand: "leybold",
    pumpType: "Oil Diffusion Vacuum Pumps",
    name: "DIFFVAC DP",
    description:
      "DIFFVAC DP series are high-performance oil diffusion pumps designed for high and ultra-high vacuum applications. These pumps provide excellent pumping speeds with reliable operation.",
    imageUrl: ["/diffvacdp.webp"],
    features: [
      "High pumping speed",
      "Large throat diameter",
      "Efficient cooling design",
      "Robust construction",
      "Integrated heaters",
      "Multi-stage jet assembly",
    ],
    applications: [
      "Metallurgical processing",
      "Vacuum coating",
      "Space simulation",
      "Research systems",
      "Industrial processes",
      "High vacuum systems",
    ],
    technicalSpecs: {
      "Nominal Pumping Speed": "2,000-50,000 l/s",
      "Ultimate Pressure": "< 5 × 10⁻⁷ mbar",
      "Warm-up Time": "30-45 minutes",
      "Cooling Water Flow": "100-500 l/h",
      "Heater Power": "1.5-12 kW",
      "Oil Capacity": "0.5-15 L",
    },
    additionalFeatures: [
      "Safety thermal switches",
      "Water cooling jackets",
      "Ejector stages",
      "Fractionating jet system",
      "Optional accessories",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley28",
    brand: "leybold",
    pumpType: "Oil Diffusion Vacuum Pumps",
    name: "DIFFVAC OB",
    description:
      "DIFFVAC OB series are optimized oil diffusion pumps for industrial applications requiring consistent high vacuum performance. These pumps feature advanced design for improved efficiency.",
    imageUrl: ["/diffvacob.webp"],
    features: [
      "Optimized jet design",
      "Quick heating system",
      "Enhanced cooling",
      "Industrial design",
      "Reliable operation",
      "Easy maintenance",
    ],
    applications: [
      "Vacuum furnaces",
      "Vacuum metallurgy",
      "Glass coating",
      "Optical coating",
      "Display manufacturing",
      "Large chamber evacuation",
    ],
    technicalSpecs: {
      "Pumping Speed": "6,000-30,000 l/s",
      "Ultimate Pressure": "< 1 × 10⁻⁶ mbar",
      "Heater Power": "3-9 kW",
      "Cold Cap Temperature": "< 35°C",
      "Maximum Throughput": "12 mbar l/s",
      "Operating Range": "10⁻³-10⁻⁷ mbar",
    },
    additionalFeatures: [
      "Foreline baffle",
      "Temperature monitoring",
      "Water flow protection",
      "Multiple mounting options",
      "Industrial controls",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley29",
    brand: "leybold",
    pumpType: "Cryogenic Systems",
    name: "COOLPAK e",
    description:
      "COOLPAK e is an advanced cryogenic refrigeration system designed for high-vacuum applications. It provides reliable cooling for cryopumps and cold traps with excellent efficiency and minimal maintenance requirements.",
    imageUrl: ["/coolpake.webp"],
    features: [
      "Closed-cycle refrigeration",
      "Microprocessor control",
      "Low power consumption",
      "High cooling capacity",
      "Compact design",
      "Low maintenance",
    ],
    applications: [
      "Semiconductor processing",
      "Optical coating",
      "Space simulation",
      "Research facilities",
      "Particle accelerators",
      "Cryopump operation",
    ],
    technicalSpecs: {
      "Cooling Power": "100-2000W at 80K",
      "Temperature Range": "20-80K",
      "Power Consumption": "3-15 kW",
      "Operating Time": "> 40,000 hours",
      "Control Interface": "Digital display",
      "Cooling Method": "Helium cycle",
    },
    additionalFeatures: [
      "Automatic temperature control",
      "System diagnostics",
      "Remote monitoring",
      "Safety interlocks",
      "Multiple compressor options",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley30",
    brand: "leybold",
    pumpType: "Cryogenic Systems",
    name: "COOLPOWER e",
    description:
      "COOLPOWER e series are compact cold heads designed for cryogenic cooling applications. These units provide efficient and reliable cooling for various scientific and industrial applications.",
    imageUrl: ["/coolpowere.webp"],
    features: [
      "High reliability design",
      "Compact cold head",
      "Low vibration",
      "Variable temperature control",
      "Multiple stage options",
      "Flexible mounting",
    ],
    applications: [
      "Detector cooling",
      "Infrared sensors",
      "Superconducting devices",
      "Crystal growing",
      "Laboratory research",
      "Cryogenic testing",
    ],
    technicalSpecs: {
      "Cooling Capacity": "5-200W at 80K",
      "Base Temperature": "< 20K",
      "Power Input": "1-5 kW",
      "Cool-down Time": "30-60 minutes",
      Weight: "5-15 kg",
      "Service Interval": "12,000 hours",
    },
    additionalFeatures: [
      "Easy installation",
      "Low maintenance needs",
      "Temperature sensors",
      "Various configurations",
      "Compatibility options",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "ley31",
    brand: "leybold",
    pumpType: "Cryogenic Systems",
    name: "COOLVAC e",
    description:
      "COOLVAC e series are high-performance cryopumps providing ultra-high vacuum capabilities. These pumps offer superior pumping speeds and extremely low ultimate pressures for demanding applications.",
    imageUrl: ["/coolvace.webp", "/pump7.png"],
    features: [
      "High pumping speed",
      "Ultra-high vacuum capable",
      "Advanced regeneration",
      "Smart control system",
      "Multiple size options",
      "Integrated temperature monitoring",
    ],
    applications: [
      "Thin film deposition",
      "Space simulation",
      "Optical coating",
      "Semiconductor processing",
      "Research facilities",
      "High-energy physics",
    ],
    technicalSpecs: {
      "Pumping Speed": "2,000-60,000 l/s",
      "Ultimate Pressure": "< 1 × 10⁻¹¹ mbar",
      "Cool-down Time": "90-180 minutes",
      "Regeneration Time": "120-240 minutes",
      "Cold Surface Area": "0.03-0.5 m²",
      "Operating Position": "Any orientation",
    },
    additionalFeatures: [
      "Fast regeneration cycle",
      "Advanced control options",
      "Safety interlocks",
      "Remote operation capability",
      "Multiple mounting configurations",
    ],
    datasheetUrl: "",
    videoId: "",
  },

  // Welch

  {
    id: "wel1",
    brand: "welch",
    pumpType: "Scroll Vacuum Pump",
    name: "SCSpro 15",

    description:
      "The SCSpro 15 is a high-performance, oil-free scroll vacuum pump designed for clean applications requiring deep vacuum. It features advanced scroll technology with a hermetically sealed design and low maintenance requirements.",
    imageUrl: ["/scroll.jpg"],
    features: [
      "100% oil-free operation",
      "Completely dry running scroll pump",
      "Hermetically sealed design",
      "Low noise level < 55 dB(A)",
      "Minimal maintenance required",
      "Compact footprint",
      "Air-cooled operation",
      "Maintenance-free bearings",
    ],
    applications: [
      "Mass Spectrometry",
      "Electron Microscopy",
      "Analytical Instruments",
      "Load Lock Systems",
      "Glove Box Applications",
      "Research Laboratories",
      "Clean Vacuum Systems",
      "Helium Recovery",
    ],
    technicalSpecs: {
      "Pumping Speed": "15.0 m³/h (250 L/min)",
      "Ultimate Pressure": "< 0.01 mbar (< 7.5 x 10⁻³ Torr)",
      "Power Consumption": "0.75 kW",
      "Noise Level": "< 55 dB(A)",
      "Operating Temperature": "10-40°C",
      Weight: "32 kg",
      "Inlet Connection": "NW 25",
      "Exhaust Connection": "NW 16",
    },
    additionalFeatures: [
      "Start/stop switch with power indicator",
      "Hours counter for maintenance tracking",
      "Integrated thermal protection",
      "Vibration-isolated mounting",
      "CE certified",
      "Two-year warranty",
    ],
    datasheetUrl:
      "https://www.welchvacuum.com/products/scroll-vacuum-pumps/scspro-15",
    videoId: "",
  },
  {
    id: "wel2",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 4",
    description:
      "The CRVpro 4 is a compact direct-drive rotary vane vacuum pump offering reliable performance for laboratory applications. It features an integrated gas ballast valve and efficient oil recirculation system.",
    imageUrl: ["/crvpro4.jpg"],
    features: [
      "Direct drive motor technology",
      "Integrated gas ballast valve",
      "Oil mist eliminator",
      "Efficient oil recirculation",
      "Anti-suckback valve",
      "Compact footprint",
      "Low noise operation",
      "Easy maintenance design",
    ],
    applications: [
      "Vacuum filtration",
      "Gel drying",
      "Vacuum ovens",
      "Rotary evaporation",
      "Vacuum concentration",
      "Small freeze drying",
      "Laboratory distillation",
      "General laboratory use",
    ],
    technicalSpecs: {
      "Pumping Speed": "4.8 m³/h (80 L/min)",
      "Ultimate Pressure": "2 × 10⁻³ mbar",
      "Motor Power": "0.25 kW",
      "Noise Level": "52 dB(A)",
      "Oil Capacity": "0.45 L",
      "Inlet Connection": "KF-16",
      Weight: "16 kg",
      Dimensions: "425 × 160 × 240 mm",
    },
    additionalFeatures: [
      "CE certified",
      "Two-year warranty",
      "Built-in hour counter",
      "Optional inlet filter",
      "Multiple voltage options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro4",
    videoId: "",
  },
  {
    id: "wel3",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 6",
    description:
      "The CRVpro 6 is a versatile rotary vane pump designed for demanding laboratory applications. It provides excellent vapor handling capabilities and reliable deep vacuum performance.",
    imageUrl: ["/crvpro6.jpg"],
    features: [
      "Enhanced vapor handling",
      "Large gas ballast valve",
      "Efficient oil separation",
      "Anti-suckback protection",
      "Forced oil lubrication",
      "Temperature monitoring",
      "Easy access maintenance ports",
      "Quiet operation",
    ],
    applications: [
      "Vacuum distillation",
      "Molecular distillation",
      "Vacuum drying",
      "Freeze drying systems",
      "Vacuum ovens",
      "Sample preparation",
      "Research applications",
      "Analytical instrumentation",
    ],
    technicalSpecs: {
      "Pumping Speed": "6.2 m³/h (103 L/min)",
      "Ultimate Pressure": "1.5 × 10⁻³ mbar",
      "Motor Power": "0.37 kW",
      "Noise Level": "54 dB(A)",
      "Oil Capacity": "0.7 L",
      "Inlet Connection": "KF-25",
      Weight: "19 kg",
      Dimensions: "440 × 175 × 250 mm",
    },
    additionalFeatures: [
      "Digital hour meter",
      "Oil level monitoring",
      "Thermal protection",
      "Multiple voltage configurations",
      "Extended warranty options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro6",
    videoId: "",
  },
  {
    id: "wel4",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 8",
    description:
      "The CRVpro 8 is a medium-capacity rotary vane pump offering superior performance for demanding laboratory applications. Features optimized oil circulation and advanced vapor handling capabilities.",
    imageUrl: ["/crvpro8.jpg"],
    features: [
      "Two-stage rotary vane design",
      "High-efficiency oil circulation",
      "Advanced gas ballast system",
      "Anti-suckback protection",
      "Quiet operation design",
      "Dual voltage motor",
      "Compact footprint",
      "Easy service access",
    ],
    applications: [
      "Vacuum distillation",
      "Freeze drying",
      "Electron microscopy",
      "Mass spectrometry",
      "Vacuum ovens",
      "Central vacuum systems",
      "Sample preparation",
      "Research applications",
    ],
    technicalSpecs: {
      "Pumping Speed": "8.4 m³/h (140 L/min)",
      "Ultimate Pressure": "1.3 × 10⁻³ mbar",
      "Motor Power": "0.45 kW",
      "Noise Level": "56 dB(A)",
      "Oil Capacity": "0.8 L",
      "Inlet Connection": "KF-25",
      Weight: "21 kg",
      Dimensions: "470 × 180 × 260 mm",
    },
    additionalFeatures: [
      "Digital vacuum display",
      "Hour meter included",
      "Thermal overload protection",
      "Multiple mounting positions",
      "Global voltage compatibility",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro8",
    videoId: "",
  },
  {
    id: "wel5",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 16",
    description:
      "The CRVpro 16 delivers high-performance vacuum for larger laboratory systems and light industrial applications. Features enhanced oil filtration and robust construction for continuous operation.",
    imageUrl: ["/crvpro16.jpg"],
    features: [
      "Two-stage pumping mechanism",
      "Large-capacity oil system",
      "Heavy-duty gas ballast",
      "Forced oil lubrication",
      "Advanced oil filtration",
      "Motor thermal protection",
      "Flexible mounting options",
      "Industrial-grade construction",
    ],
    applications: [
      "Industrial freeze drying",
      "Large vacuum ovens",
      "Vacuum furnaces",
      "Semiconductor processing",
      "Metallurgy applications",
      "Central vacuum systems",
      "Research facilities",
      "Pilot plant operations",
    ],
    technicalSpecs: {
      "Pumping Speed": "16.5 m³/h (275 L/min)",
      "Ultimate Pressure": "1.0 × 10⁻³ mbar",
      "Motor Power": "0.75 kW",
      "Noise Level": "58 dB(A)",
      "Oil Capacity": "1.0 L",
      "Inlet Connection": "KF-25",
      Weight: "29 kg",
      Dimensions: "520 × 190 × 280 mm",
    },
    additionalFeatures: [
      "Integrated pressure monitoring",
      "Advanced oil return system",
      "Extended service intervals",
      "Safety valve protection",
      "Optional water cooling",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro16",
    videoId: "",
  },
  {
    id: "wel6",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 24",
    description:
      "The CRVpro 24 is a high-capacity rotary vane pump designed for industrial and research applications requiring consistent deep vacuum. Features robust construction and advanced control options.",
    imageUrl: ["/crvpro24.jpg"],
    features: [
      "Industrial-grade design",
      "High-efficiency cooling",
      "Advanced oil management",
      "Continuous duty operation",
      "High water vapor tolerance",
      "Temperature monitoring",
      "Variable frequency drive ready",
      "Enhanced filtration system",
    ],
    applications: [
      "Industrial processes",
      "Large freeze dryers",
      "Vacuum metallurgy",
      "Chemical processing",
      "Coating systems",
      "Large vacuum chambers",
      "Production systems",
      "Research facilities",
    ],
    technicalSpecs: {
      "Pumping Speed": "24.0 m³/h (400 L/min)",
      "Ultimate Pressure": "8 × 10⁻⁴ mbar",
      "Motor Power": "1.1 kW",
      "Noise Level": "60 dB(A)",
      "Oil Capacity": "1.5 L",
      "Inlet Connection": "KF-40",
      Weight: "45 kg",
      Dimensions: "600 × 220 × 320 mm",
    },
    additionalFeatures: [
      "PLC compatibility",
      "Remote monitoring capability",
      "Predictive maintenance",
      "Energy efficiency options",
      "Extended warranty available",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro24",
    videoId: "",
  },
  {
    id: "wel7",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 30",
    description:
      "The CRVpro 30 is an industrial-grade rotary vane vacuum pump engineered for continuous heavy-duty operation. It provides reliable deep vacuum with excellent vapor handling capabilities.",
    imageUrl: ["/crvpro30.jpg"],
    features: [
      "Two-stage oil-sealed design",
      "Heavy-duty construction",
      "Large oil capacity system",
      "Advanced gas ballast control",
      "Water vapor handling capability",
      "Forced oil lubrication",
      "Industrial motor design",
      "Integrated oil filtration",
    ],
    applications: [
      "Industrial vacuum systems",
      "Large-scale freeze drying",
      "Vacuum furnaces",
      "Plasma coating",
      "Metallurgical processing",
      "Chemical processing",
      "Industrial distillation",
      "Central vacuum supply",
    ],
    technicalSpecs: {
      "Pumping Speed": "30.0 m³/h (500 L/min)",
      "Ultimate Pressure": "5 × 10⁻⁴ mbar",
      "Motor Power": "1.5 kW",
      "Noise Level": "62 dB(A)",
      "Oil Capacity": "2.0 L",
      "Inlet Connection": "KF-40",
      Weight: "62 kg",
      Dimensions: "705 × 260 × 380 mm",
    },
    additionalFeatures: [
      "Digital control interface",
      "Remote operation capability",
      "Maintenance monitoring system",
      "Variable frequency drive compatible",
      "Industrial communication protocols",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro30",
    videoId: "",
  },
  {
    id: "wel8",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 48",
    description:
      "The CRVpro 48 is a high-capacity industrial vacuum pump designed for demanding applications requiring substantial pumping speed and reliable deep vacuum performance.",
    imageUrl: ["/crvpro48.jpg"],
    features: [
      "High-capacity two-stage design",
      "Industrial cooling system",
      "Enhanced oil separation",
      "Robust gear transmission",
      "Heavy-duty motor",
      "Advanced control system",
      "Multiple protection features",
      "Continuous duty rated",
    ],
    applications: [
      "Industrial vacuum processing",
      "Large coating systems",
      "Heat treatment furnaces",
      "Industrial freeze drying",
      "Vacuum metallurgy",
      "Large chambers evacuation",
      "Production systems",
      "Multiple point extraction",
    ],
    technicalSpecs: {
      "Pumping Speed": "48.0 m³/h (800 L/min)",
      "Ultimate Pressure": "3 × 10⁻⁴ mbar",
      "Motor Power": "2.2 kW",
      "Noise Level": "65 dB(A)",
      "Oil Capacity": "3.0 L",
      "Inlet Connection": "KF-40",
      Weight: "85 kg",
      Dimensions: "820 × 300 × 420 mm",
    },
    additionalFeatures: [
      "Industry 4.0 ready",
      "Integrated monitoring system",
      "Predictive maintenance",
      "Energy efficiency optimization",
      "Extended warranty options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro48",
    videoId: "",
  },
  {
    id: "wel9",
    brand: "welch",
    pumpType: "Rotary Vane Vacuum Pumps",
    name: "CRVpro 65",
    description:
      "The CRVpro 65 is Welch's largest rotary vane vacuum pump, offering maximum pumping capacity for industrial applications requiring high throughput and consistent deep vacuum.",
    imageUrl: ["/crvpro65.jpg"],
    features: [
      "Maximum capacity design",
      "Industrial cooling system",
      "Advanced oil management",
      "Smart control interface",
      "Heavy-duty construction",
      "High-efficiency motor",
      "Multiple safety features",
      "Process optimization controls",
    ],
    applications: [
      "Large industrial systems",
      "Vacuum chamber evacuation",
      "Industrial coating lines",
      "Metallurgical processing",
      "Central vacuum systems",
      "Large-scale freeze drying",
      "Industrial heat treatment",
      "Process manufacturing",
    ],
    technicalSpecs: {
      "Pumping Speed": "65.0 m³/h (1083 L/min)",
      "Ultimate Pressure": "2 × 10⁻⁴ mbar",
      "Motor Power": "3.0 kW",
      "Noise Level": "68 dB(A)",
      "Oil Capacity": "4.0 L",
      "Inlet Connection": "KF-50",
      Weight: "115 kg",
      Dimensions: "890 × 350 × 480 mm",
    },
    additionalFeatures: [
      "Smart diagnostic system",
      "Network connectivity",
      "Data logging capability",
      "Remote monitoring and control",
      "Advanced protection features",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro65",
    videoId: "",
  },
  {
    id: "wel10",
    brand: "welch",
    pumpType: "Belt Driven",
    name: "DUOSEAL 1400",
    description:
      "The DUOSEAL 1400 is a classic belt-driven vacuum pump known for its reliability and deep ultimate vacuum. Features time-tested design and robust construction for research and industrial applications.",
    imageUrl: ["/duoseal1400.png"],
    features: [
      "Belt-driven precision mechanism",
      "Two-stage design",
      "High ultimate vacuum",
      "Cast iron construction",
      "Direct-reading vacuum gauge",
      "Air and oil ballast valves",
      "Large oil reservoir",
      "Easily serviceable design",
    ],
    applications: [
      "Research laboratories",
      "Vacuum distillation",
      "Electron microscopy",
      "Vacuum coating",
      "Mass spectrometry",
      "High vacuum systems",
      "Educational institutions",
      "Vacuum drying",
    ],
    technicalSpecs: {
      "Free Air Capacity": "160 L/min",
      "Ultimate Pressure": "1 × 10⁻⁴ Torr",
      "Motor Power": "0.33 HP",
      "Oil Capacity": "1.0 L",
      "Inlet Connection": '3/4" NPT',
      Weight: "34 kg",
      Dimensions: "460 × 200 × 300 mm",
      "Belt Type": "A-28",
    },
    additionalFeatures: [
      "Built-in vacuum gauge",
      "Metal bail handle",
      "Exhaust filter assembly",
      "Isolation valve option",
      "Multiple voltage options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/duoseal1400",
    videoId: "",
  },
  {
    id: "wel11",
    brand: "welch",
    pumpType: "Belt Driven",
    name: "DUOSEAL 1402",
    description:
      "The DUOSEAL 1402 is a high-capacity belt-driven vacuum pump designed for applications requiring both deep vacuum and higher pumping speeds. Offers exceptional reliability and maintainability.",
    imageUrl: ["/duoseal1402.png"],
    features: [
      "High-capacity design",
      "Dual-stage operation",
      "Precision belt drive",
      "Heavy-duty construction",
      "Large oil sight glass",
      "Multiple inlet ports",
      "Enhanced cooling fins",
      "Vibration isolation system",
    ],
    applications: [
      "Vacuum ovens",
      "Industrial processes",
      "Research systems",
      "Vacuum impregnation",
      "Analytical instruments",
      "Metallurgical processing",
      "Laboratory applications",
      "Vacuum drying systems",
    ],
    technicalSpecs: {
      "Free Air Capacity": "200 L/min",
      "Ultimate Pressure": "1 × 10⁻⁴ Torr",
      "Motor Power": "0.5 HP",
      "Oil Capacity": "1.5 L",
      "Inlet Connection": '1" NPT',
      Weight: "41 kg",
      Dimensions: "510 × 230 × 320 mm",
      "Belt Type": "A-30",
    },
    additionalFeatures: [
      "Multi-port manifold option",
      "Heavy-duty motor mount",
      "Oil return system",
      "Belt tension adjustment",
      "Extended warranty available",
    ],
    datasheetUrl: "https://welchvacuum.com/products/duoseal1402",
    videoId: "",
  },
  {
    id: "wel12",
    brand: "welch",
    pumpType: "Belt Driven",
    name: "DUOSEAL 1405",
    description:
      "The DUOSEAL 1405 is a research-grade belt-driven vacuum pump offering exceptional ultimate vacuum and reliability. Features precision manufacturing and robust design for demanding laboratory applications.",
    imageUrl: ["/duoseal1405.png"],
    features: [
      "Research-grade performance",
      "Precision belt drive system",
      "Two-stage operation",
      "High pumping efficiency",
      "Protected drive mechanism",
      "Low maintenance design",
      "Quiet operation",
      "Oil mist elimination",
    ],
    applications: [
      "Research vacuum systems",
      "Advanced laboratories",
      "Mass spectrometry",
      "Electron microscopy",
      "Thin film deposition",
      "High vacuum processes",
      "Material research",
      "Analytical instrumentation",
    ],
    technicalSpecs: {
      "Free Air Capacity": "275 L/min",
      "Ultimate Pressure": "5 × 10⁻⁵ Torr",
      "Motor Power": "0.75 HP",
      "Oil Capacity": "2.0 L",
      "Inlet Connection": '1-1/4" NPT',
      Weight: "48 kg",
      Dimensions: "550 × 250 × 350 mm",
      "Belt Type": "A-32",
    },
    additionalFeatures: [
      "Precision vacuum gauge",
      "Enhanced oil filtration",
      "Vibration isolation",
      "Safety features",
      "Research-grade construction",
    ],
    datasheetUrl: "https://welchvacuum.com/products/duoseal1405",
    videoId: "",
  },
  {
    id: "wel13",
    brand: "welch",
    pumpType: "Belt Driven",
    name: "DUOSEAL 1376",
    description:
      "The DUOSEAL 1376 is a heavy-duty belt-driven vacuum pump engineered for industrial applications. Provides reliable performance with excellent durability and serviceability.",
    imageUrl: ["/duoseal1376.png"],
    features: [
      "Industrial-grade construction",
      "High-capacity belt drive",
      "Enhanced cooling system",
      "Large oil reservoir",
      "Heavy-duty bearings",
      "Industrial motor",
      "Dual-stage design",
      "Robust base frame",
    ],
    applications: [
      "Industrial processing",
      "Vacuum heat treating",
      "Large vacuum systems",
      "Production environments",
      "Metallurgical processes",
      "Vacuum drying",
      "Industrial coating",
      "Process manufacturing",
    ],
    technicalSpecs: {
      "Free Air Capacity": "400 L/min",
      "Ultimate Pressure": "1 × 10⁻⁴ Torr",
      "Motor Power": "1.0 HP",
      "Oil Capacity": "3.0 L",
      "Inlet Connection": '1-1/2" NPT',
      Weight: "65 kg",
      Dimensions: "650 × 300 × 400 mm",
      "Belt Type": "A-36",
    },
    additionalFeatures: [
      "Industrial controls",
      "Heavy-duty filtration",
      "Temperature monitoring",
      "Easy servicing design",
      "Industrial warranty",
    ],
    datasheetUrl: "https://welchvacuum.com/products/duoseal1376",
    videoId: "",
  },
  {
    id: "wel14",
    brand: "welch",
    pumpType: "Belt Driven",
    name: "DUOSEAL 1397",
    description:
      "The DUOSEAL 1397 is a specialized belt-driven vacuum pump designed for continuous operation in demanding environments. Features enhanced reliability and maintenance-friendly design.",
    imageUrl: ["/duoseal1397.png"],
    features: [
      "Continuous duty design",
      "Specialized belt system",
      "Advanced oil cooling",
      "Protected drive assembly",
      "High-efficiency motor",
      "Enhanced sealing",
      "Precision machining",
      "Serviceability features",
    ],
    applications: [
      "Continuous processing",
      "Industrial systems",
      "Production vacuum",
      "Process equipment",
      "Manufacturing lines",
      "Vacuum packaging",
      "Material handling",
      "Industrial drying",
    ],
    technicalSpecs: {
      "Free Air Capacity": "350 L/min",
      "Ultimate Pressure": "2 × 10⁻⁴ Torr",
      "Motor Power": "0.85 HP",
      "Oil Capacity": "2.5 L",
      "Inlet Connection": '1-1/4" NPT',
      Weight: "55 kg",
      Dimensions: "600 × 275 × 375 mm",
      "Belt Type": "A-34",
    },
    additionalFeatures: [
      "Extended operation features",
      "Enhanced cooling system",
      "Maintenance indicators",
      "Industrial-grade filtration",
      "Continuous duty rating",
    ],
    datasheetUrl: "https://welchvacuum.com/products/duoseal1397",
    videoId: "",
  },
  {
    id: "wel15",
    brand: "welch",
    pumpType: "Belt Driven",
    name: "DUOSEAL 1399",
    description:
      "The DUOSEAL 1399 is a premium belt-driven vacuum pump offering superior performance for critical applications. Features advanced design elements and precision engineering for reliable operation.",
    imageUrl: ["/duoseal1399.png"],
    features: [
      "Premium performance design",
      "Advanced belt drive system",
      "Precision balancing",
      "Superior oil management",
      "Enhanced vacuum control",
      "Thermal management",
      "Quality construction",
      "Optimized efficiency",
    ],
    applications: [
      "Critical processes",
      "Research facilities",
      "Advanced manufacturing",
      "Precision coating",
      "Specialized vacuum systems",
      "Quality control labs",
      "Development facilities",
      "Scientific applications",
    ],
    technicalSpecs: {
      "Free Air Capacity": "300 L/min",
      "Ultimate Pressure": "8 × 10⁻⁵ Torr",
      "Motor Power": "0.8 HP",
      "Oil Capacity": "2.2 L",
      "Inlet Connection": '1-1/4" NPT',
      Weight: "52 kg",
      Dimensions: "575 × 265 × 365 mm",
      "Belt Type": "A-33",
    },
    additionalFeatures: [
      "Premium components",
      "Advanced monitoring",
      "Precision controls",
      "Extended reliability features",
      "Professional installation options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/duoseal1399",
    videoId: "",
  },
  {
    id: "wel16",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2522",
    description:
      "The WOB-L 2522 is a compact, oil-free piston vacuum pump designed for general laboratory applications. Features maintenance-free operation and reliable performance.",
    imageUrl: ["/wob2522.jpg"],
    features: [
      "Oil-free operation",
      "Maintenance-free design",
      "Compact footprint",
      "Low noise level",
      "Chemical resistant",
      "Portable construction",
      "Thermal protection",
      "Quick starting",
    ],
    applications: [
      "General laboratory use",
      "Vacuum filtration",
      "Sample preparation",
      "Fluid aspiration",
      "Small vacuum ovens",
      "Educational labs",
      "Air sampling",
      "Basic research",
    ],
    technicalSpecs: {
      "Free Air Capacity": "37 L/min",
      "Ultimate Pressure": "100 Torr",
      "Motor Power": "0.12 HP",
      "Noise Level": "< 55 dB(A)",
      Weight: "4.5 kg",
      "Inlet Connection": '1/4" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Self-lubricating pistons",
      "Built-in muffler",
      "Carrying handle",
      "Rubber feet",
      "Multiple voltage options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2522",
    videoId: "",
  },
  {
    id: "wel17",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2534",
    description:
      "The WOB-L 2534 delivers increased capacity for medium-duty laboratory applications with oil-free operation. Ideal for applications requiring higher flow rates.",
    imageUrl: ["/wob2534.jpg"],
    features: [
      "Higher capacity design",
      "Oil-free technology",
      "Durable construction",
      "Enhanced cooling",
      "Chemical resistant head",
      "Low vibration",
      "Easy maintenance",
      "Reliable starting",
    ],
    applications: [
      "Rotary evaporation",
      "Vacuum filtration",
      "Laboratory processes",
      "Small freeze dryers",
      "Vacuum concentrators",
      "Research equipment",
      "Environmental sampling",
      "Quality control",
    ],
    technicalSpecs: {
      "Free Air Capacity": "46 L/min",
      "Ultimate Pressure": "75 Torr",
      "Motor Power": "0.16 HP",
      "Noise Level": "57 dB(A)",
      Weight: "5.9 kg",
      "Inlet Connection": '3/8" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Enhanced durability",
      "Improved flow design",
      "Noise reduction",
      "Temperature management",
      "Service indicators",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2534",
    videoId: "",
  },
  {
    id: "wel18",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2546",
    description:
      "The WOB-L 2546 is a high-performance oil-free piston pump designed for demanding laboratory applications requiring consistent vacuum performance.",
    imageUrl: ["/wob2546.jpg"],
    features: [
      "High performance design",
      "Advanced piston system",
      "Heavy-duty motor",
      "Enhanced cooling system",
      "Chemical resistant materials",
      "Continuous duty rating",
      "Low maintenance needs",
      "Reliable operation",
    ],
    applications: [
      "Analytical instruments",
      "Vacuum ovens",
      "Laboratory automation",
      "Process equipment",
      "Environmental testing",
      "Research facilities",
      "Quality assurance",
      "Scientific applications",
    ],
    technicalSpecs: {
      "Free Air Capacity": "67 L/min",
      "Ultimate Pressure": "50 Torr",
      "Motor Power": "0.25 HP",
      "Noise Level": "59 dB(A)",
      Weight: "8.2 kg",
      "Inlet Connection": '1/2" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Advanced air cooling",
      "Performance monitoring",
      "Extended life design",
      "Multiple mounting options",
      "Global certification",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2546",
    videoId: "",
  },
  {
    id: "wel19",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2561",
    description:
      "The WOB-L 2561 provides industrial-grade performance with oil-free operation for demanding applications requiring both deep vacuum and reliability.",
    imageUrl: ["/wob2561.jpg"],
    features: [
      "Industrial design",
      "High vacuum capability",
      "Robust construction",
      "Advanced cooling",
      "Long-life components",
      "Low vibration system",
      "Easy maintenance access",
      "Protection features",
    ],
    applications: [
      "Industrial processes",
      "Laboratory systems",
      "Automated equipment",
      "Research facilities",
      "Product testing",
      "Quality control",
      "Process automation",
      "Scientific instruments",
    ],
    technicalSpecs: {
      "Free Air Capacity": "83 L/min",
      "Ultimate Pressure": "40 Torr",
      "Motor Power": "0.33 HP",
      "Noise Level": "62 dB(A)",
      Weight: "11.4 kg",
      "Inlet Connection": '1/2" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Industrial grade components",
      "Enhanced reliability",
      "System monitoring",
      "Safety features",
      "Extended warranty options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2561",
    videoId: "",
  },
  {
    id: "wel20",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2562",
    description:
      "The WOB-L 2562 is engineered for high-capacity applications requiring oil-free operation with superior reliability and performance.",
    imageUrl: ["/wob2562.jpg"],
    features: [
      "High capacity design",
      "Premium components",
      "Enhanced durability",
      "Efficient cooling",
      "Advanced protection",
      "Low maintenance",
      "Quiet operation",
      "Industrial reliability",
    ],
    applications: [
      "Process systems",
      "Manufacturing",
      "Laboratory automation",
      "Research equipment",
      "Testing facilities",
      "Production lines",
      "Quality assurance",
      "Development labs",
    ],
    technicalSpecs: {
      "Free Air Capacity": "98 L/min",
      "Ultimate Pressure": "35 Torr",
      "Motor Power": "0.4 HP",
      "Noise Level": "63 dB(A)",
      Weight: "13.6 kg",
      "Inlet Connection": '3/4" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Premium efficiency motor",
      "Advanced monitoring",
      "Extended service life",
      "Multiple protection features",
      "Global certifications",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2562",
    videoId: "",
  },
  {
    id: "wel21",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2563",
    description:
      "The WOB-L 2563 delivers maximum performance in the series with advanced features for demanding industrial and research applications.",
    imageUrl: ["/wob2563.jpg"],
    features: [
      "Maximum performance",
      "Industrial construction",
      "Advanced cooling system",
      "Heavy-duty components",
      "Enhanced protection",
      "Continuous operation",
      "Low maintenance design",
      "Premium reliability",
    ],
    applications: [
      "Industrial processes",
      "Large laboratories",
      "Production systems",
      "Research facilities",
      "Testing equipment",
      "Process automation",
      "Manufacturing",
      "Development centers",
    ],
    technicalSpecs: {
      "Free Air Capacity": "127 L/min",
      "Ultimate Pressure": "30 Torr",
      "Motor Power": "0.5 HP",
      "Noise Level": "65 dB(A)",
      Weight: "15.9 kg",
      "Inlet Connection": '3/4" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Industrial controls",
      "System diagnostics",
      "Enhanced durability",
      "Multiple safety features",
      "Extended service intervals",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2563",
    videoId: "",
  },
  {
    id: "wel22",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2567",
    description:
      "The WOB-L 2567 is a specialized high-flow pump designed for applications requiring substantial air movement with oil-free operation.",
    imageUrl: ["/wob2567.jpg"],
    features: [
      "High flow capacity",
      "Specialized design",
      "Robust construction",
      "Enhanced cooling",
      "Advanced protection",
      "Industrial grade",
      "Easy maintenance",
      "Reliable performance",
    ],
    applications: [
      "Large systems",
      "Process equipment",
      "Industrial automation",
      "Manufacturing lines",
      "Testing facilities",
      "Research centers",
      "Development labs",
      "Production systems",
    ],
    technicalSpecs: {
      "Free Air Capacity": "185 L/min",
      "Ultimate Pressure": "65 Torr",
      "Motor Power": "0.75 HP",
      "Noise Level": "68 dB(A)",
      Weight: "18.2 kg",
      "Inlet Connection": '1" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Flow optimization",
      "Temperature management",
      "Advanced monitoring",
      "Protection systems",
      "Industrial reliability",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2567",
    videoId: "",
  },
  {
    id: "wel23",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2581",
    description:
      "The WOB-L 2581 is an advanced dual-head pump offering high capacity and deep vacuum capabilities for demanding applications.",
    imageUrl: ["/wob2581.jpg"],
    features: [
      "Dual-head design",
      "Enhanced performance",
      "Industrial construction",
      "Advanced cooling",
      "Premium components",
      "High reliability",
      "Low maintenance",
      "continuous duty",
    ],
    applications: [
      "Industrial systems",
      "Large laboratories",
      "Production equipment",
      "Research facilities",
      "Process automation",
      "Testing centers",
      "Manufacturing",
      "Development facilities",
    ],
    technicalSpecs: {
      "Free Air Capacity": "235 L/min",
      "Ultimate Pressure": "25 Torr",
      "Motor Power": "1.0 HP",
      "Noise Level": "70 dB(A)",
      Weight: "22.7 kg",
      "Inlet Connection": '1" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Dual head synchronization",
      "Advanced monitoring",
      "System protection",
      "Industrial controls",
      "Extended service life",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2581",
    videoId: "",
  },
  {
    id: "wel24",
    brand: "welch",
    pumpType: "WOB-L Piston Pumps",
    name: "WOB-L Piston Pump 2585",
    description:
      "The WOB-L 2585 represents the pinnacle of the series with maximum capacity and advanced features for demanding industrial applications.",
    imageUrl: ["/wob2585.jpg"],
    features: [
      "Maximum capacity design",
      "Premium construction",
      "Advanced cooling system",
      "Industrial grade components",
      "Enhanced protection features",
      "Continuous operation",
      "Easy serviceability",
      "Superior reliability",
    ],
    applications: [
      "Large industrial systems",
      "Production facilities",
      "Process automation",
      "Manufacturing lines",
      "Research centers",
      "Testing facilities",
      "Development labs",
      "Quality control",
    ],
    technicalSpecs: {
      "Free Air Capacity": "285 L/min",
      "Ultimate Pressure": "20 Torr",
      "Motor Power": "1.5 HP",
      "Noise Level": "72 dB(A)",
      Weight: "27.2 kg",
      "Inlet Connection": '1-1/4" NPT',
      "Operating Temperature": "5-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Industrial control system",
      "Advanced diagnostics",
      "Multiple protection features",
      "Enhanced durability",
      "Global certification",
    ],
    datasheetUrl: "https://welchvacuum.com/products/wob2585",
    videoId: "",
  },
  {
    id: "wel25",
    brand: "welch",
    pumpType: "Diaphragm Pumps",
    name: "Chemistry Diaphragm Pumps (MPC)",
    description:
      "The Chemistry Diaphragm Pumps (MPC) series are specifically designed for handling aggressive chemicals and vapors. Features 100% oil-free operation with PTFE-coated components for maximum chemical resistance.",
    imageUrl: ["/chem.jpg"],
    features: [
      "100% oil-free operation",
      "PTFE-coated diaphragms",
      "Chemical resistant design",
      "Vapor handling capability",
      "Safety features",
      "Maintenance-free motor",
      "Easy service access",
      "Integrated gas ballast",
    ],
    applications: [
      "Chemical laboratories",
      "Solvent handling",
      "Rotary evaporation",
      "Acid processing",
      "Vacuum filtration",
      "Gel drying",
      "Sample preparation",
      "Research facilities",
    ],
    technicalSpecs: {
      "Free Air Capacity": "20-60 L/min",
      "Ultimate Pressure": "8 mbar",
      "Chemical Resistance": "High",
      "Motor Power": "0.1-0.25 kW",
      "Noise Level": "< 52 dB(A)",
      "Operating Temperature": "10-40°C",
      "Protection Class": "IP54",
      Weight: "8.5-12 kg",
    },
    additionalFeatures: [
      "Solvent recovery option",
      "Multiple head configurations",
      "Temperature monitoring",
      "Inlet catch pot available",
      "Safety overpressure valve",
    ],
    datasheetUrl: "https://welchvacuum.com/products/mpc",
    videoId: "",
  },
  {
    id: "wel26",
    brand: "welch",
    pumpType: "Diaphragm Pumps",
    name: "Standard Duty Diaphragm Pumps (MP)",
    description:
      "The Standard Duty Diaphragm Pumps (MP) provide reliable vacuum for general laboratory applications. These pumps offer maintenance-free operation with excellent reliability.",
    imageUrl: ["/standard.jpg"],
    features: [
      "Oil-free operation",
      "Long-life diaphragms",
      "Compact design",
      "Low maintenance",
      "Quiet operation",
      "Universal voltage motor",
      "Portable construction",
      "Thermal protection",
    ],
    applications: [
      "General laboratory use",
      "Vacuum filtration",
      "Sample preparation",
      "Educational labs",
      "Small vacuum ovens",
      "Basic research",
      "Quality control",
      "Testing facilities",
    ],
    technicalSpecs: {
      "Free Air Capacity": "15-40 L/min",
      "Ultimate Pressure": "75 mbar",
      "Motor Power": "0.08-0.15 kW",
      "Noise Level": "< 50 dB(A)",
      Weight: "5-8 kg",
      "Inlet Connection": '3/8" NPT',
      "Operating Temperature": "10-40°C",
      "Duty Cycle": "Continuous",
    },
    additionalFeatures: [
      "Built-in muffler",
      "Carrying handle",
      "Rubber feet",
      "Multiple voltage options",
      "CE certification",
    ],
    datasheetUrl: "https://welchvacuum.com/products/mp",
    videoId: "",
  },
  {
    id: "wel27",
    brand: "welch",
    pumpType: "Diaphragm Pumps",
    name: "Laboratory Vacuum Systems (LVS)",
    description:
      "Laboratory Vacuum Systems (LVS) are integrated solutions combining diaphragm pump technology with advanced control features for precise vacuum management in laboratory environments.",
    imageUrl: ["/laboratory.jpg"],
    features: [
      "Integrated vacuum control",
      "Digital display",
      "Programmable operation",
      "Chemical resistant design",
      "Multiple port options",
      "Energy efficient",
      "Network capability",
      "Safety features",
    ],
    applications: [
      "Research laboratories",
      "Analytical procedures",
      "Multiple workstations",
      "Chemistry labs",
      "Process development",
      "Quality assurance",
      "Educational facilities",
      "R&D centers",
    ],
    technicalSpecs: {
      "System Capacity": "30-100 L/min",
      "Pressure Range": "1-800 mbar",
      "Control Accuracy": "±1 mbar",
      "Power Consumption": "0.2-0.5 kW",
      Interface: "Touch screen",
      Communication: "RS-232/USB",
      "Protection Class": "IP54",
      "Number of Ports": "2-8",
    },
    additionalFeatures: [
      "Data logging",
      "Process automation",
      "Remote monitoring",
      "Customizable programs",
      "Maintenance tracking",
    ],
    datasheetUrl: "https://welchvacuum.com/products/lvs",
    videoId: "",
  },
  {
    id: "wel28",
    brand: "welch",
    pumpType: "Diaphragm Pumps",
    name: "GEMINI Environmental Monitoring Pumps",
    description:
      "GEMINI Environmental Monitoring Pumps are specialized diaphragm pumps designed for air sampling and environmental monitoring applications. Features precise flow control and reliable operation.",
    imageUrl: ["/gemini.jpg"],
    features: [
      "Precise flow control",
      "Battery operation option",
      "Weather-resistant design",
      "Long-term operation",
      "Flow measurement",
      "Data recording",
      "Portable design",
      "Multiple sampling modes",
    ],
    applications: [
      "Air quality monitoring",
      "Environmental sampling",
      "Gas analysis",
      "Workplace monitoring",
      "Emissions testing",
      "Field research",
      "Compliance testing",
      "Scientific studies",
    ],
    technicalSpecs: {
      "Flow Rate": "1-20 L/min",
      "Flow Control": "±2% accuracy",
      "Battery Life": "24 hours",
      Weight: "4.5 kg",
      "Operating Temperature": "-10 to 45°C",
      "Memory Capacity": "10,000 data points",
      Display: "LCD with backlight",
      "Protection Rating": "IP65",
    },
    additionalFeatures: [
      "GPS location logging",
      "Wireless connectivity",
      "Environmental sensors",
      "Auto calibration",
      "Extended battery packs",
    ],
    datasheetUrl: "https://welchvacuum.com/products/gemini",
    videoId: "",
  },
  {
    id: "wel29",
    brand: "welch",
    pumpType: "Vacuum Gauges and Controllers",
    name: "Digital Vacuum Gauge DVG-1",
    description:
      "The DVG-1 is a precision digital vacuum gauge designed for accurate measurement across a wide range. Features easy calibration and multiple pressure unit display options.",
    imageUrl: ["/welch-dvg1.jpg"],
    features: [
      "Wide measurement range",
      "Digital display",
      "Multiple pressure units",
      "Battery operated",
      "Auto shut-off function",
      "Compact design",
      "Easy calibration",
      "Durable construction",
    ],
    applications: [
      "Laboratory vacuum systems",
      "Research facilities",
      "Quality control",
      "Process monitoring",
      "Vacuum pump testing",
      "Leak detection",
      "System diagnostics",
      "Educational demonstrations",
    ],
    technicalSpecs: {
      "Measurement Range": "1-760 Torr",
      Accuracy: "±0.5% full scale",
      "Response Time": "< 0.5 seconds",
      "Battery Life": "100 hours",
      "Display Resolution": "0.1 Torr",
      "Operating Temperature": "10-40°C",
      Weight: "250g",
      Dimensions: "120 × 65 × 30 mm",
    },
    additionalFeatures: [
      "Backlit display",
      "Low battery indicator",
      "Zero function",
      "Peak hold capability",
      "Protective rubber boot",
    ],
    datasheetUrl: "https://welchvacuum.com/products/dvg1",
    videoId: "",
  },
  {
    id: "wel30",
    brand: "welch",
    pumpType: "Vacuum Gauges and Controllers",
    name: "Vacuum Controller VC-1",
    description:
      "The VC-1 is an advanced vacuum controller offering precise pressure control and automation capabilities for laboratory vacuum applications.",
    imageUrl: ["/welch-vc1.jpg"],
    features: [
      "PID control system",
      "Programmable setpoints",
      "Data logging capability",
      "Multiple sensor inputs",
      "Digital display",
      "RS-232 interface",
      "Alarm functions",
      "Solenoid valve control",
    ],
    applications: [
      "Vacuum distillation",
      "Rotary evaporation",
      "Freeze drying",
      "Process automation",
      "Research systems",
      "Quality assurance",
      "Laboratory automation",
      "Vacuum oven control",
    ],
    technicalSpecs: {
      "Control Range": "1-760 Torr",
      "Control Accuracy": "±1% full scale",
      "Response Time": "100ms",
      "Power Supply": "100-240 VAC",
      "Display Type": "LCD graphic",
      Communication: "RS-232/USB",
      "Relay Outputs": "4 programmable",
      "Memory Storage": "100 programs",
    },
    additionalFeatures: [
      "Touch screen interface",
      "Process visualization",
      "Data export capability",
      "Multiple language support",
      "Password protection",
    ],
    datasheetUrl: "https://welchvacuum.com/products/vc1",
    videoId: "",
  },
  {
    id: "wel31",
    brand: "welch",
    pumpType: "Turbomolecular Pumps and Systems",
    name: "TurboVac 50",
    description:
      "The TurboVac 50 is a compact turbomolecular pump designed for high and ultra-high vacuum applications. Features integrated electronics and advanced bearing technology for reliable operation.",
    imageUrl: ["/welch-turbo50.jpg"],
    features: [
      "Integrated controller",
      "Hybrid bearing system",
      "Compact design",
      "High compression ratio",
      "Low vibration",
      "Air cooling",
      "Temperature monitoring",
      "Protection features",
    ],
    applications: [
      "Mass spectrometry",
      "Electron microscopy",
      "Surface analysis",
      "Research systems",
      "Load lock chambers",
      "Analytical instruments",
      "High vacuum systems",
      "Scientific equipment",
    ],
    technicalSpecs: {
      "Nominal Pumping Speed": "50 L/s (N₂)",
      "Ultimate Pressure": "< 1 × 10⁻⁹ mbar",
      "Compression Ratio": "> 1 × 10⁸ (N₂)",
      "Rotation Speed": "90,000 rpm",
      "Start-up Time": "< 2 minutes",
      "Power Consumption": "60W nominal",
      Weight: "2.8 kg",
      "Inlet Flange": "DN 63 CF",
    },
    additionalFeatures: [
      "USB interface",
      "Remote operation",
      "Status monitoring",
      "Vent valve control",
      "Backing pump interface",
    ],
    datasheetUrl: "https://welchvacuum.com/products/turbovac50",
    videoId: "",
  },
  {
    id: "wel32",
    brand: "welch",
    pumpType: "Oil Diffusion Pumps and Systems",
    name: "DifPump 100",
    description:
      "The DifPump 100 is a high-performance oil diffusion pump designed for industrial and research applications requiring ultra-high vacuum. Features optimized jet stages and efficient cooling design.",
    imageUrl: ["/welch-difpump.jpg"],
    features: [
      "Multi-stage jet assembly",
      "Efficient water cooling",
      "High throughput capability",
      "Fractionating design",
      "Built-in safety features",
      "Rapid warm-up",
      "Optimized jet geometry",
      "Industrial-grade heaters",
    ],
    applications: [
      "Vacuum metallurgy",
      "Space simulation",
      "Thin film deposition",
      "Research systems",
      "Industrial coating",
      "High energy physics",
      "Electron beam welding",
      "Semiconductor processing",
    ],
    technicalSpecs: {
      "Nominal Pumping Speed": "3000 L/s",
      "Ultimate Pressure": "< 1 × 10⁻⁷ mbar",
      "Critical Backing Pressure": "0.55 mbar",
      "Heater Power": "3 kW",
      "Cooling Water Flow": "120 L/h",
      "Oil Charge": "3 L",
      "Warm-up Time": "30 minutes",
      "Body Temperature": "< 50°C",
    },
    additionalFeatures: [
      "Temperature monitoring",
      "Cooling water protection",
      "High vacuum valve option",
      "Backing pump interlock",
      "Maintenance indicators",
    ],
    datasheetUrl: "https://welchvacuum.com/products/difpump100",
    videoId: "",
  },
  {
    id: "wel33",
    brand: "welch",
    pumpType: "Fluid Aspiration Systems",
    name: "BioVac 225",
    description:
      "The BioVac 225 is a compact fluid aspiration system designed for safe and efficient handling of biological and chemical waste in laboratory environments.",
    imageUrl: ["/welch-biovac.jpg"],
    features: [
      "Chemical resistant design",
      "Automatic overflow protection",
      "Quick-connect fittings",
      "Integrated vacuum control",
      "Quiet operation",
      "Portable design",
      "Safety features",
      "Easy waste disposal",
    ],
    applications: [
      "Cell culture work",
      "Tissue culture",
      "Clinical laboratories",
      "Research facilities",
      "Microbiological work",
      "Sample preparation",
      "Waste collection",
      "Medical labs",
    ],
    technicalSpecs: {
      "Collection Capacity": "2.5 L",
      "Vacuum Range": "0-600 mmHg",
      "Flow Rate": "15 L/min",
      "Noise Level": "< 50 dB(A)",
      "Filter Type": "0.2μm hydrophobic",
      "Power Supply": "100-240 VAC",
      Dimensions: "25 × 25 × 40 cm",
      Weight: "3.2 kg",
    },
    additionalFeatures: [
      "Autoclavable collection bottle",
      "Level sensor",
      "Multiple bottle options",
      "Hand operator",
      "Foot pedal control",
    ],
    datasheetUrl: "https://welchvacuum.com/products/biovac225",
    videoId: "",
  },
  {
    id: "wel34",
    brand: "welch",
    pumpType: "Fluid Aspiration Systems",
    name: "BioVac 320",
    description:
      "The BioVac 320 is an advanced fluid aspiration system with increased capacity and enhanced features for demanding laboratory environments.",
    imageUrl: ["/welch-biovac320.jpg"],
    features: [
      "High capacity system",
      "Dual bottle configuration",
      "Advanced safety features",
      "Digital vacuum control",
      "Multiple user ports",
      "Chemical resistant materials",
      "Continuous operation",
      "Status monitoring",
    ],
    applications: [
      "Multi-user laboratories",
      "Core facilities",
      "Biotech companies",
      "Research institutions",
      "Clinical labs",
      "Pharmaceutical labs",
      "Quality control",
      "Production facilities",
    ],
    technicalSpecs: {
      "Collection Capacity": "4.0 L",
      "Vacuum Range": "0-700 mmHg",
      "Flow Rate": "20 L/min",
      "Noise Level": "< 52 dB(A)",
      "Filter Type": "0.2μm hydrophobic",
      "Power Supply": "100-240 VAC",
      Dimensions: "30 × 30 × 45 cm",
      Weight: "4.5 kg",
    },
    additionalFeatures: [
      "Quick-change bottles",
      "Multiple user ports",
      "Digital display",
      "Remote operation",
      "Maintenance alerts",
    ],
    datasheetUrl: "https://welchvacuum.com/products/biovac320",
    videoId: "",
  },
  {
    id: "wel35",
    brand: "welch",
    pumpType: "Vacuum Networks",
    name: "VACNET Laboratory Vacuum Network",
    description:
      "The VACNET Laboratory Vacuum Network is an integrated vacuum distribution system designed for multi-user laboratory environments. Features smart controls and efficient vacuum delivery to multiple points of use.",
    imageUrl: ["/welch-vacnet.jpg"],
    features: [
      "Centralized vacuum generation",
      "Smart load balancing",
      "Multi-point distribution",
      "Pressure monitoring",
      "Energy optimization",
      "Network management",
      "Zone control",
      "Remote monitoring capability",
    ],
    applications: [
      "Research laboratories",
      "University facilities",
      "Analytical labs",
      "R&D centers",
      "Quality control labs",
      "Pharmaceutical labs",
      "Clinical laboratories",
      "Teaching facilities",
    ],
    technicalSpecs: {
      "System Capacity": "100-2000 L/min",
      "Vacuum Range": "1-750 Torr",
      "Number of Points": "4-32",
      "Control System": "PLC based",
      Interface: "Touch screen HMI",
      Communication: "Ethernet/IP",
      "Power Supply": "208-480 VAC",
      "Protection Class": "IP54",
    },
    additionalFeatures: [
      "Usage analytics",
      "Preventive maintenance alerts",
      "Energy consumption tracking",
      "Zone scheduling",
      "Emergency shutdown",
    ],
    datasheetUrl: "https://welchvacuum.com/products/vacnet",
    videoId: "",
  },
  {
    id: "wel36",
    brand: "welch",
    pumpType: "Vacuum Pump Oils",
    name: "DirecTorr Gold Vacuum Pump Oil",
    description:
      "DirecTorr Gold is a premium-grade vacuum pump oil specifically formulated for high-performance vacuum applications. Features superior oxidation resistance and excellent vapor pressure characteristics.",
    imageUrl: ["/welch-oil.jpg"],
    features: [
      "Superior oxidation stability",
      "Low vapor pressure",
      "Extended service life",
      "High temperature stability",
      "Excellent lubrication",
      "Chemical resistance",
      "Low maintenance",
      "High purity formulation",
    ],
    applications: [
      "Rotary vane pumps",
      "Diffusion pumps",
      "Industrial vacuum systems",
      "Research equipment",
      "High vacuum applications",
      "Laboratory pumps",
      "Process equipment",
      "Manufacturing systems",
    ],
    technicalSpecs: {
      "Vapor Pressure": "< 1 × 10⁻⁷ Torr at 25°C",
      Viscosity: "ISO VG 68",
      "Pour Point": "-15°C",
      "Flash Point": "230°C",
      "Specific Gravity": "0.87",
      "Available Sizes": "1L, 4L, 20L",
      "Storage Life": "5 years",
      Color: "Clear amber",
    },
    additionalFeatures: [
      "Anti-wear additives",
      "Moisture resistance",
      "Thermal stability",
      "Compatible with most pumps",
      "Easy disposal options",
    ],
    datasheetUrl: "https://welchvacuum.com/products/directorrGold",
    videoId: "",
  },
  {
    id: "wel_gauge_1",
    brand: "welch",
    pumpType: "Vacuum Gauges and Controllers",
    name: "SMART Digital Vacuum Gauge",
    description:
      "High-precision digital vacuum gauge with wide measurement range and easy-to-read display",
    imageUrl: ["/welch-smart-gauge.jpg"],
    features: [
      "Wide measurement range from atmosphere to 10⁻³ Torr",
      "Large digital display",
      "Battery operated",
      "USB data output",
      "Calibration certificate included",
      "Compact design",
    ],
    applications: [
      "Laboratory vacuum measurement",
      "Process monitoring",
      "Quality control",
      "Research applications",
      "Educational demonstrations",
      "System diagnostics",
    ],
    technicalSpecs: {
      "Measurement Range": "760 to 10⁻³ Torr",
      Accuracy: "±10% of reading",
      "Response Time": "< 1 second",
      "Battery Life": "50 hours continuous",
      "Display Type": "LCD with backlight",
      "Operating Temperature": "10-40°C",
    },
    additionalFeatures: [
      "Auto shut-off function",
      "Low battery indicator",
      "Multiple pressure units",
      "Protective rubber boot",
      "Data logging capability",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wel_gauge_2",
    brand: "welch",
    pumpType: "Vacuum Gauges and Controllers",
    name: "DigiVac Model 215",
    description:
      "Advanced digital vacuum controller with integrated process control capabilities",
    imageUrl: ["/welch-215.jpg"],
    features: [
      "Dual sensor capability",
      "PID process control",
      "Data logging",
      "Multiple setpoints",
      "RS-232 interface",
      "Graphical display",
    ],
    applications: [
      "Vacuum distillation",
      "Freeze drying",
      "Vacuum furnaces",
      "Process automation",
      "Research systems",
      "Industrial control",
    ],
    technicalSpecs: {
      "Measurement Range": "760 to 10⁻⁴ Torr",
      "Control Accuracy": "±1% full scale",
      "Display Resolution": "0.1 mTorr",
      "Power Supply": "90-240 VAC",
      "Relay Outputs": "3 programmable",
      Communication: "RS-232/USB",
    },
    additionalFeatures: [
      "Programmable setpoints",
      "Trend graphing",
      "Alarm functions",
      "Multiple sensor compatibility",
      "Password protection",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wel_hv_1",
    brand: "welch",
    pumpType: "High Vacuum Systems",
    name: "HVS 2000 Series",
    description:
      "Advanced high vacuum pumping system combining turbomolecular and backing pumps for precision vacuum applications",
    imageUrl: ["/welch-hvs2000.jpg"],
    features: [
      "Integrated turbo and backing pump",
      "Advanced vacuum control",
      "Automatic protection features",
      "Touch screen interface",
      "Data logging capability",
      "Compact design",
    ],
    applications: [
      "Mass spectrometry",
      "Electron microscopy",
      "Thin film coating",
      "Research laboratories",
      "Surface analysis",
      "Vacuum testing",
    ],
    technicalSpecs: {
      "Ultimate Pressure": "< 1 × 10⁻⁷ mbar",
      "Pumping Speed": "250-1000 L/s",
      "Inlet Flange": "DN 100 CF",
      "Control Interface": '7" Touch Screen',
      "Power Rating": "1.5 kW",
      "Operating Temperature": "10-40°C",
    },
    additionalFeatures: [
      "Remote operation",
      "Multiple gauge options",
      "Customizable setpoints",
      "USB data export",
      "Maintenance tracking",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wel_cr_1",
    brand: "welch",
    pumpType: "Chemical-Resistant Pumps",
    name: "CRVpro Chemical Series",
    description:
      "Chemical-resistant vacuum pumps designed for aggressive vapor handling and corrosive applications",
    imageUrl: ["/welch-crvpro-chem.jpg"],
    features: [
      "PTFE wetted surfaces",
      "Corrosion-resistant design",
      "Gas ballast valve",
      "Solvent recovery capability",
      "Temperature monitoring",
      "Safety features",
    ],
    applications: [
      "Chemical processing",
      "Solvent recovery",
      "Acid processing",
      "Laboratory distillation",
      "Rotary evaporation",
      "Chemical extraction",
    ],
    technicalSpecs: {
      "Pumping Speed": "4-65 m³/h",
      "Ultimate Pressure": "< 2 × 10⁻³ mbar",
      "Chemical Resistance": "High",
      "Power Consumption": "0.25-2.2 kW",
      "Noise Level": "< 65 dB(A)",
      "Protection Class": "IP54",
    },
    additionalFeatures: [
      "Acid-resistant coating",
      "Exhaust condensate trap",
      "Inlet catch pot",
      "Temperature protection",
      "Easy maintenance design",
    ],
    datasheetUrl: "",
    videoId: "",
  },
  {
    id: "wel_sp_1",
    brand: "welch",
    pumpType: "Specialty Vacuum Pumps",
    name: "ChemStar 1400N",
    description:
      "The ChemStar 1400N is a specialized chemical-duty vacuum pump designed for handling aggressive vapors and corrosive chemicals. Features PTFE-coated wetted surfaces and advanced corrosion protection.",
    imageUrl: ["/welch-chemstar.jpg"],
    features: [
      "PTFE wetted surfaces",
      "Chemical-resistant design",
      "Dual-stage operation",
      "Advanced vapor handling",
      "Corrosion protection",
      "Safety features",
      "Temperature monitoring",
      "Easy maintenance",
    ],
    applications: [
      "Chemical processing",
      "Solvent recovery",
      "Acid handling",
      "Chemical distillation",
      "Laboratory processes",
      "Research applications",
      "Analytical procedures",
      "Pilot plant operations",
    ],
    technicalSpecs: {
      "Free Air Capacity": "142 L/min",
      "Ultimate Pressure": "2 × 10⁻³ mbar",
      "Chemical Resistance": "High",
      "Motor Power": "0.4 kW",
      "Operating Temperature": "10-40°C",
      Weight: "28.5 kg",
      "Inlet Connection": "KF-25",
      "Protection Class": "IP54",
    },
    additionalFeatures: [
      "Solvent recovery system",
      "Gas ballast valve",
      "Temperature protection",
      "Inlet trap options",
      "Extended warranty",
    ],
    datasheetUrl: "https://welchvacuum.com/products/chemstar1400n",
    videoId: "",
  },
  {
    id: "wel_med_1",
    brand: "welch",
    pumpType: "Medical Vacuum Systems",
    name: "MPC 301Z",
    description:
      "The MPC 301Z is a medical-grade vacuum pump system designed for clinical and healthcare applications. Features clean operation and reliable performance with infection control considerations.",
    imageUrl: ["/welch-mpc301z.jpg"],
    features: [
      "Medical-grade construction",
      "Clean room compatible",
      "Low noise operation",
      "Infection control design",
      "Continuous duty rated",
      "Easy sterilization",
      "Multiple safety features",
      "Status monitoring",
    ],
    applications: [
      "Clinical laboratories",
      "Healthcare facilities",
      "Medical research",
      "Diagnostic equipment",
      "Surgical suction",
      "Dental applications",
      "Hospital systems",
      "Medical devices",
    ],
    technicalSpecs: {
      "Pumping Speed": "30 L/min",
      "Ultimate Pressure": "20 mbar",
      "Noise Level": "< 45 dB(A)",
      "Power Consumption": "0.15 kW",
      "Filter Grade": "HEPA H14",
      "Operating Temperature": "10-40°C",
      "Protection Class": "IP54",
      Certification: "ISO 13485",
    },
    additionalFeatures: [
      "Bacterial filtration",
      "Automatic drain system",
      "Contamination protection",
      "Service indicators",
      "Emergency backup",
    ],
    datasheetUrl: "https://welchvacuum.com/products/mpc301z",
    videoId: "",
  },
  {
    id: "wel_sys_1",
    brand: "welch",
    pumpType: "Vacuum Systems",
    name: "CRVpro Multi-User System",
    description:
      "The CRVpro Multi-User System is an integrated vacuum solution designed for multiple simultaneous users in laboratory environments. Features smart controls and efficient vacuum distribution.",
    imageUrl: ["/welch-crvsystem.jpg"],
    features: [
      "Multi-user capability",
      "Smart vacuum control",
      "Energy optimization",
      "Individual port control",
      "Network monitoring",
      "Modular design",
      "Remote operation",
      "Data logging",
    ],
    applications: [
      "Research laboratories",
      "Educational facilities",
      "Analytical labs",
      "Quality control",
      "Process development",
      "Multi-user facilities",
      "Core facilities",
      "Testing centers",
    ],
    technicalSpecs: {
      "System Capacity": "250 L/min",
      "Vacuum Range": "1-800 mbar",
      "User Ports": "4-12",
      "Control Interface": "Touch screen",
      Communication: "Ethernet",
      "Power Rating": "1.5 kW",
      Protection: "IP54",
      "Operating Mode": "24/7",
    },
    additionalFeatures: [
      "Usage tracking",
      "Scheduling system",
      "Maintenance alerts",
      "Zone management",
      "Process automation",
    ],
    datasheetUrl: "https://welchvacuum.com/products/crvpro-system",
    videoId: "",
  },
  // End of Welch product series
];
